...
# Hardcoded single admin user — fine for a portfolio project
fake_admin = {
    "username": "admin",
...
def login(form_data: OAuth2PasswordRequestForm = Depends()):
    if form_data.username != fake_admin["username"] or not verify_password(form_data.password, fake_admin["hashed_password"]):
        raise HTTPException(status_code=401, detail="Incorrect username or password")
    access_token = create_access_token(data={"sub": form_data.username})
    return {"access_token": access_token, "token_type": "bearer"}

@app.put('/edit/{paitent_id}')
def update_paitent(paitent_id: str, paitent_update: updatePaitent, current_user: str = Depends(get_current_user)):
...
def delete_paitent(paitent_id: str, current_user: str = Depends(get_current_user)):
